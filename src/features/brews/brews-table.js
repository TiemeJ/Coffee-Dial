...
export const createBrewsTableModule = ({
    ...
    const toggleQuickFilter = async (e) => {
        ...
        const presetRows = presets.length
            ? presets.map((preset) => {
                ...
                return `<div class="flex items-center gap-1">
                    ...
                    <button type="button" data-quick-preset-delete="${presetId}" class="w-8 h-8 rounded border border-coffee-200 dark:border-[#44403c] bg-white dark:bg-[#1c1917] text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center justify-center" title="Delete">
        ...
        html += '<button type="button" data-quick-tab="settings" class="quick-tab-btn px-2 py-1 rounded text-xs font-semibold text-coffee-600 dark:text-[#a8a29e]" title="Settings"><i class="fa-solid fa-gear"></i></button>';
        html += '</div>';
        html += '<button type="button" data-action-click="toggleQuickFilter(event)" class="w-7 h-7 rounded border border-coffee-200 dark:border-[#44403c] bg-white dark:bg-[#1c1917] text-coffee-600 dark:text-[#a8a29e] hover:bg-coffee-50 dark:hover:bg-[#34302e] flex items-center justify-center" title="Close"><i class="fa-solid fa-xmark text-[11px]"></i></button>';
        html += '</div>';
        ...
        html += '<select id="quickFilterSortAddSelect" class="w-full px-2 py-1.5 rounded border border-coffee-200 dark:border-[#44403c] bg-white dark:bg-[#1c1917] text-xs text-coffee-900 dark:text-white">';
        ...
        if (isAccordionSettingsMode) {
            ...
            for (let index = 0; index < 3; index += 1) {
                html += `<div class="grid grid-cols-[70px_1fr] items-center gap-2">
                    ...
                    <select data-quick-accordion-field-index="${index}" class="w-full px-2 py-1.5 rounded border border-coffee-200 dark:border-[#44403c] bg-white dark:bg-[#1c1917] text-xs text-coffee-900 dark:text-white">`;
            ...
            <div class="pt-2 space-y-3">
                ...
                <div class="bg-coffee-50 dark:bg-[#1c1917] rounded-xl p-4 border border-coffee-100 dark:border-[#44403c]">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        ${hasGraph
                            ? `<button data-action-click="brewsOpenCard('${brew.id}', event); openCardGraphModal(event, true)" class="text-coffee-400 hover:text-coffee-700 dark:text-[#78716c] dark:hover:text-white transition-colors" title="View Graph"><i class="fa-solid fa-chart-line"></i></button>`
                            : ''}
                    </div>
